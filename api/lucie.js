const SHEET_ID = '1mIticCUh6WbKRUqTfqtEtJK0yojIt0uutVv6-oben80';
const TABS = {
  summary:      2080491094,
  ga:           1757386842,
  facebook:     1449017236,
  googleAds:    148251842,
  reservations: 1480972037,
  perfectVenue: 1263324238,
  emails:       1339932462,
  emailSent:    130153647,
  fbLeads:      1763932413,
  seo:          138927766,
...
async function fetchCSV(gid) {
  ...
  if (text.trim().startsWith('<!DOCTYPE')) throw new Error(`Tab gid=${gid} returned HTML — tab may be deleted`);
  return text.split('\n').filter(l => l.trim());
}

const safe = (fn, fallback) => fn().catch(e => { console.error(e.message); return fallback; });
...
async function processFacebook() {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const isLeadCampaign = campaign.toLowerCase().startsWith('[lead]') || campaign.toLowerCase().startsWith('[leads]');
    ...
    const empty = () => ({ spend:0, leadSpend:0, otSpend:0, reach:0, impressions:0, clicks:0, resultsCount:0, otReservations:0 });
  ...
  dailyArr.forEach(d => { d.ctr = d.impressions > 0 ? +(d.clicks/d.impressions*100).toFixed(2) : 0; });
...
async function processGoogleAds() {
  ...
  const campMonthlyArr = Object.values(campMonthly).sort((a,b) => a.month.localeCompare(b.month));
...
async function processReservations() {
  // New tab layout (same as Carbon Bar GID 830275616):
  ...
  // col[19]=Size, col[20]=Status, col[61]=POS Paid, col[86]=First Visit Date
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const guestId = email || phone || null;
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!dateKey) continue;
    if (parseInt(dateKey.slice(0,4)) < 2020) continue;
    ...
    const ltpc = cleanNum(col[61]); // POS Paid = column BJ (0-indexed col 61)
  ...
  return {
    summary: { total, totalCovers, firstVisit, done, cancelled, noShow,
...
async function processPerfectVenue() {
  ...
  parseCSVLine(lines[0] || ''); // headers — not used but validates tab
  for (let i = 1; i < lines.length; i++) {
    const col = parseCSVLine(lines[i]);
    if (!col[1]) continue;
    ...
    const proposalTotal = cleanNum(col[40]); // Proposal Total = col 40
    const totalPaid     = cleanNum(col[44]); // Total Paid = col 44
    const balanceDue    = cleanNum(col[45]); // Balance Due = col 45
    const budget        = cleanNum(col[17]); // Budget = col 17
    ...
    const pvEmail = (col[9]||'').trim().toLowerCase(); // Contact Email = col 9
    ...
    if (createdOn && createdOn.match(/\d+\/\d+\/\d+/)) {
      ...
      if (y) {
        ...
        if (pvEmail) {
          ...
          pvGuestIndex[pvEmail].push(dateKey);
        ...
        if (isContactForm) {
          ...
          if (status==='Completed'||status==='Confirmed') {
            ...
            monthly[key].proposalTotal += proposalTotal; pvDaily[dateKey].proposalTotal += proposalTotal;
...
async function processFacebookLeads(reservationGuestIndex, pvGuestIndex = {}, pvRevenueIndex = {}, resRevenueIndex = {}) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!daily[dateKey]) daily[dateKey] = { date: dateKey, leads: 0, matched: 0, newGuests: 0, returning: 0, pePvMatched: 0 };
    if (!monthly[monthKey]) monthly[monthKey] = { month: monthKey, leads: 0, matched: 0, newGuests: 0, returning: 0 };
    ...
    if (email && pvGuestIndex[email]) {
      ...
      if (hasPVMatch) {
        pvMatched++;
        if (isPECampaign) {
          pePvMatched++; daily[dateKey].pePvMatched++;
...
export default async function handler(_req, res) {
  ...
  try {
    const [summary, ga, facebook, googleAds, reservations, perfectVenue, email, seo] = await Promise.all([
      safe(processSummary,    { daily: [], monthly: [] }),
      ...
      safe(processReservations, { summary:{}, days:{}, sizes:[], monthly:[], daily:[], firstVisitDaily:[], repeatDaily:[], guestIndex:{} }),
